...
INTEGER :: N, i, j, ip, jp, ii, nargc, ifi, nfi
REAL*8 :: epsilon, fi
...
INTEGER  LWORK,INFO
INTEGER :: iterator, max_iteration
CHARACTER*50 text
LOGICAL :: conv
...
allocate(psi(0:N))
allocate(psi_old(0:N))
allocate(pos(0:N))
allocate(potential(0:N))
allocate(potential_old(0:N))
...
OPEN(1, FILE="2D/psi_init.dat")
do i=1,N
    write(1, '(200e20.12)') pos(i)/fnm2au, initial_psi(pos(i),0.0d0,sigma)
enddo
CLOSE(1) 

tmp=0.0
do i=0,N
   tmp=tmp+2.0*pi*(initial_psi(pos(i),0.0d0,sigma))**2*pos(i)*dx
enddo
print *, 'Initial function normalization', tmp

do i=0,N
   psi(i)=initial_psi(pos(i),0.0d0,sigma)
enddo
...
do i=0,N
   ...
   nfi = int(2*pi/dfi)
      ...
      xp=ip*dx*cos(fi)
      yp=ip*dx*sin(fi)
      potential(i)=potential(i)+1/epsilon*abs(psi(ip))**2/sqrt((pos(i)-xp)**2+(0-yp)**2+4*d**2)*dx*ip*dfi*dx
   enddo
enddo
   enddo
...
!!!!!!!!!!!!  solve Schrodinger eqution !!!!!!!!!!!!
   ...
   do i=1,N
      if (i.eq.1) then
            A(i,i)=1/m/2.0/dx**2+(-potential(i))+1.0/2.0/m/2.0/pos(i)/dx
            A(i,i+1)=-1.0/2.0/m/dx**2-1.0/2.0/m/2.0/pos(i)/dx
      else if (i.eq.(N)) then
            A(i,i)=1/m/dx**2+(-potential(i))
            A(i,i-1)=-1.0/2.0/m/dx**2+1.0/2.0/m/2.0/pos(i)/dx
      else
            A(i,i)=1/m/dx**2+(-potential(i))
            A(i,i+1)=-1.0/2.0/m/dx**2-1.0/2.0/m/2.0/pos(i)/dx
   ...
   tmp=0.0
   do i=0,N
      tmp=tmp+2.0*pi*abs(psi(i))**2*pos(i)*dx
   enddo
   ...
   enddo
    ...
    WRITE(text,'(a3,i0.0,a4)') '2D/pot',iterator,'.dat'
   OPEN(1, FILE=text)
   do i=0,N
   ...
   WRITE(text,'(a3,i0.0,a4)') '2D/psi',iterator,'.dat'
   OPEN(1, FILE=text)
   do i=1,N
   ...
   OPEN(1, FILE="2D/energy.dat", position="append")
...
OPEN(1, FILE="2D/pot.dat")
do i=0,N
    write(1, '(200e20.12)') pos(i)/fnm2au, -potential(i)/feV2au
enddo
CLOSE(1)

OPEN(1, FILE="2D/psi.dat")
do i=1,N
    write(1, '(200e20.12)') pos(i)/fnm2au, psi(i)
enddo
CLOSE(1) 
...
function initial_psi(x,y,sigma) result(val)
    implicit none

    real(8), intent(in) :: x, y, sigma
    real(8) :: val
    real(8), parameter :: pi = 3.141592653589793d0
    real(8), parameter :: fnm2au = 18.897261d0

    val = 1.0d0/(pi*sigma**2)**(1.0d0/2.0d0) * exp(-(x**2 + y**2)/(2.0d0*sigma**2))
