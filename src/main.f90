PROGRAM MAIN
    ...
    REAL*8, ALLOCATABLE :: potential_z_new(:)
    REAL*8, ALLOCATABLE :: potential_z_old(:)
    ...
    ALLOCATE(potential_z_new(nz_1d))
    ALLOCATE(potential_z_old(nz_1d))
    ...
    ! stage 2: dielectric
    CALL GET_EPSILON(potential_z, eps_0, &
                       nx, ny, &
                       nz_1d, dz_1d, &
                       nz_3d, dz_3d, &
                       epsilon)
    ...
    DO iter = 1, MAX_ITER_SCF
        ...
        DO k=1, Nz_3d
            DO j=1, Ny
                DO i=1, Nx
                    potential(i,j,k)=potential(i,j,k)-potential_z_new(k)
                END DO
            END DO
        END DO
        ...
        electric_field, density, nz_3d, dz_3D, potential_z_new)
        ! potential_z = beta*potential_z_new + (1.d0-beta)*potential_z_old
        CALL GET_EPSILON(potential_z_new, eps_0, &
                       nx, ny, &
                       nz_1d, dz_1d, &
                       nz_3d, dz_3d, &
                       epsilon)
        potential_z_old = potential_z_new
    END DO
    ...
    DEALLOCATE(potential_z_new)
    DEALLOCATE(potential_z_old)
    DEALLOCATE(epsilon)
