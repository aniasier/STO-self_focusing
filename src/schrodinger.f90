MODULE SCHRODINGER
    ...
    SUBROUTINE IMAGINARY_TIME(potential, Nx, Ny, Nz, dx, dz, m1, m2, x0, y0, z0)
        IMPLICIT NONE
        REAL*8, INTENT(IN) :: potential(:,:,:)
        REAL*8, INTENT(IN) :: dx, dz, m1, m2, x0, y0, z0
        ...
        REAL*8 :: x, y, z, sigma, energy, energy_old, tol, norm, dt
        ...
        DO i=2, Nx-1
            DO  j=2, Ny-1
                DO k=2, Nz-1
                    ham(i,j,k) = -1/(2*m1)*(( psi(i+1,j,k)+psi(i-1,j,k) + psi(i,j+1,k)+psi(i,j-1,k)-&
        ...
        DO iter =1, MAX_TIME
            ...
            DO i=2, Nx-1
                DO  j=2, Ny-1
                    DO k=2, Nz-1
                        ham(i,j,k) = -1/(2*m1)*(( psi_new(i+1,j,k)+psi_new(i-1,j,k) + psi_new(i,j+1,k)+&
                        psi_new(i,j-1,k)-4.d0*psi_new(i,j,k))/(dx**2) + (psi_new(i,j,k+1)+psi_new(i,j,k-1)&
                         - 2.d0*psi_new(i,j,k) )/dz**2) - potential(i,j,k) * psi_new(i,j,k)
                    END DO
                END DO
            END DO
